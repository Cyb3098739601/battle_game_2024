...
cybTank::cybTank(GameCore *game_core, uint32_t id, uint32_t player_id)
...
void cybTank::TankMove(float move_speed, float rotate_angular_speed) {
...
bool cybTank::IsHit(glm::vec2 position) const {
  position = WorldToLocal(position);
  return position.x > -0.5f && position.x < 0.5f && position.y > -0.5f &&
         position.y < 0.5f && position.x + position.y < 1.6f &&
         position.y - position.x < 1.6f;
}

const char *cybTank::UnitName() const {
  return "cyb";
}

const char *cybTank::Author() const {
  return "caoyb";
}
}  // namespace battle_game::unit
